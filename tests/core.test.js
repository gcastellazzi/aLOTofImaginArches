...
test('every example reproduces its stored block centroids and areas', () => {
  ...
  assert.ok(checked >= 15, `only ${checked} examples checked`);
});
...
test('an inconsistent example is detected, never silently redrawn', () => {
  ...
  // The six known bad ones. If this number moves, the data changed and the
  // application's warning path needs looking at.
  assert.equal(flagged.length, 6,
    `expected 6 inconsistent examples, found ${flagged.length}: ` +
